...
Scenario: FAB-1335: Resilient Kafka Orderer and Brokers
    Given the Kafka Default Replication Factor environment variable is 4
    And the configtx Orderer Batchsize environment variable is 30
    And the configtx Orderer BatchTimeout environment variable is 20 minutes
    And a bootstrapped orderer network of type kafka
...
@skip
Scenario: FAB-x4686: Test taking down all kafka brokers and bringing back first 3
  Given I have a bootstrapped fabric network of type kafka
  And I wait "60" seconds
  When a user deploys chaincode at path "github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02" with ["init","a","1000","b","2000"] with name "mycc"
  And I wait "5" seconds
  ...
  When a user queries on the chaincode named "mycc" with args ["query","a"]
  Then a user receives expected response of 970
  # Currently returns 980 because of SERVICE UNAVAILABLE with invoke
